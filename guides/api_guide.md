...
1. API возвращает использует HTTP статусы в ответах. Успешная обработка запроса - 20x, редирект - 30x,
   ошибка - 4xx.
1. API информативен. Для успешной обработки запроса помимо ответа API может вернуть параметры запроса.  В случае ошибки
   возвращается описание и массив ошибок валидатора, может быть ссылка на документацию по API и рекомендации
   как ошибки избежать.
1. Работать с API будут прежде всего разработчики и мы хотим максимально им в этом помочь.
1. API использует HTTP заголовки. Версионирование, запрос конкретных feature, желаемый типа данных, передача
...
1. API это не (только лишь) CRUD над ORM, сущности API должны отражать бизнес-процессы. При этом может существовать
   и низкоуровневый слой API для манипуляции с базой.
...
В случае ошибки ответ содержит поле `message` с описание проблемы, массив `errors` может содержать подробности
от валидаторов.
...
Коды ответа 200, 201, 204 для успешно обработанных запросов. При ошибках обработки 400, ошибках приложения 420,
ошибках валидации 422, ошибках доступа 401, 403, 404 (когда необходимо).
...
Все ошибки, возникающие при обработке валидного запроса, возвращаются с кодом 420 Going To Hell Error. Что-то пошло
не так на стороне сервера, но это не фатальная ошибка типа 500, а ошибка бизнес-логики.
...
1. Basic схема может использоваться в обычном режиме (login:password) для авторизации некоторых приватных ручек,
...
При нехватке прав на запрашиваемый ресурс API обычно отвечает 403 Not Authorized. В некоторых случаях можно
возвращать 404 Not Found, чтобы скрыть сам факт наличия конкретного ресурса (`/clients/nonexistent` и
`/client/existing` должны оба ответить 404, если у пользователя нет прав).

### Паджинация

Максимальный размер страницы данных, который возвращает API, ограничен и может устанавливаться параметром `per_page`
(дефолт 20, максимум 200). В ответе в разделе `meta` содержатся ссылки `url`, `next_url`, `prev_url` и `last_url`
на текущую, следующую, предыдущую и последнюю страницы выборки.

Чтобы запросить полный набор данных, надо запрашивать следующую страницу по `meta.next_url`, пока он присутствует в
ответе.

#### Постраничная паджинация

Страница выбирается параметром `page` (отсчет начинается с 1).
...
Так `page=1` может всегда указывать на самые последние записи, однако два запроса одной и той же страницы могут веррнуть
разный набор данных.

Параметр `before=[object_uuid]` или `after=[object_uuid]` задает элемент коллекции и ее сортировку. Before - элементы с
датой создания меньше, чем у выбораного, сортировка по убыванию. After - элементы с датой создания больше, чем у
выбранного, сортировка по возрастаннию.
...
Базовый content-type для API это `application/json` - формат ответа json, все дополнительные опции и версия
API в дефолте.
...
* compact - например, не разворачивать связанные объекты, а оставлять их ссылками, либо не включать в выдачу
