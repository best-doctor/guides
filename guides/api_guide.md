...
(дефолт 20, максимум 200). Страница выбирается параметром `page` (отсчет начинается с 1), при этом в ответе в
разделе `meta` содержатся ссылки `url`, `next_url` и `last_ulr` на текущую, следующую и последнюю страницы выборки.
...
Чтобы запросить полный набор данных, надо запрашивать следующую страницу по `meta.next_url`, пока он
