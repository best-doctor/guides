...
1. API возвращает использует HTTP статусы в ответах. Успешная обработка запроса - 20x, редирект - 30x,
   ошибка в запросе или при его обработке - 4xx.
...
1. API это не (только лишь) CRUD над ORM, сущности API должны отражать бизнес-процессы. В дополнение к этому может существовать
...
* `query` - разобранный запрос, включая дефолтные значения необязательных полей, не заданных в запросе. Отражает полные входные данные обработчика запроса.

Каждый объект содержит ссылку на себя в атрибуте url и ссылки на связанные ресурсы в атрибутах, заканчивающихся на _url.

В случае ошибки ответ содержит поле `message` с человекочитаемым описанием проблемы, массив `errors` может содержать подробности
от валидаторов.
...
Коды ответа 200, 201, 204 возвращаются для успешно обработанных запросов. При ошибках обработки 400, ошибках приложения 420,
ошибках валидации 422, ошибках доступа 401, 403, 404 (когда необходимо).

### Передача параметров

Параметры для GET-запросов передаются в url, а также в query string после `?`. Остальные запросы принимают параметры в url, а также в json payload.
...
не так с точки зрения сервера, но это не фатальная ошибка сервера типа 500, а ошибка, предусмотренная бизнес-логикой.
...
Данные для аутентификации клиента передаются в заголовке "Authorization: (basic|bearer|token) XXX"
...
* Явную или неявную фильтрацию/ограниченный объем ответа (список доступных ресурсов для public может быть меньше, чем для пациента или HR).
* Ошибку 401 Authorization Required

При успешной аутентиификации, но нехватке прав на конкретный запрашиваемый ресурс API обычно отвечает 403 Not Authorized. В некоторых случаях стоит
возвращать 404 Not Found, чтобы скрыть сам факт наличия конкретного ресурса (клиенту, не имеющему должных прав, обращения к `/clients/nonexistent` и
`/client/existing` должны оба вернуть 404).

### Паджинация (страничный вывод)

Максимальный размер страницы данных, который возвращает API, ограничен сервером, и может запрашиваться в параметре `per_page`
...
Чтобы запросить полный набор данных, клиенту следует запрашивать следующую страницу по `meta.next_url`, пока он присутствует в
ответе.

В разделе `meta` может присутствовать поле `total`, обозначающие общее количество элементов данных, и поле `pages`, обозначающее количество страниц (размера `per_page`).
...
Базовый content-type для API это `application/json` - формат ответа json, кодировка UTF-8, все дополнительные опции и версия
API в дефолте.

Расширенный content-type выглядит так: `application/vnd.bestdoctor.[version].[option[-verbose]][+format]`

Параметр format сейчас json, вряд ли он станет xml когда-нибудь. yml возможен.
