...
Единообразие существенно снижает когнитивную сложность восприятия кода. Все наши проекты должны быть в одном стиле во всем: наименовании, структуре и тд.
...
[eslint-config-bestdoctor](https://github.com/best-doctor/eslint-config-bestdoctor) используем во всех проектах, при необходимости добавляем специфичные для проекта правила/плагины (например штуки про
...
размер. Это может быть гифка спиннера, которая используется в кнопках и после input в форме. В этом случае мы создаем компонент Spinner в папке common, в котором будет гифка с навешанными стилями.
...
2. Разделение компонента на логику в контейнере и представление в файле компонента позволяет:
   ...
   2. изолированно тестировать;
3. Разбиение компонента на части/подкомпоненты, вынос функций в utils/libs позволяет:
   1. изолированно тестировать;
   2. уменьшает размер файлов, что существенно упрощает их восприятие;
4. Легко перемещать компонент в структуре приложения, либо между приложениями, при необходимости. Особенно если
...
2. `MyComponentView.tsx` в котором находится верстка и стили компонента.
    ...
    2. в нем могут находиться простые функции преобразования данных для отображения, либо объявление хендлеров
...
1. `MyComponentsContainer.tsx` в котором:
    ...
    3. 1. тут же могут находиться функции/интерфейсы которые шарятся между компонентом и контейнером, либо
...
2. component parts - файлы с версткой-стилями некоторых кусков `MyComponent`, которые выносятся дабы уменьшить размер
...
3. subcomponents - папки имеющие такую же структуру как компонент, то есть index файл, файл компонента, контейнер
...
4. `(utils/libs).ts` - файл с функциями используемыми в компоненте или контейнере, которые выносятся дабы уменьшить
...
- в libs находятся функции напрямую связанные с бизнес логикой приложения. Например обработка ответов бека
  сложные вычисления специфичных вещей, вроде подбора клиник по франшизе и тд.
...
Как правило, если функция принимает слишком много параметров - нужно ее разбивать. Исключения - функции не содержащиелогику в себе, например для передаче кучи полей на бек в одном запросе и тд. При этом функция может содержать асинхронный код который должен выполнится в момент ее вызова. То есть мы не можем заранее получить какой-то фрагмент готовых данных и просто передать его в эту функцию. Но, мы можем байндить функции с их параметрами и вызывать их в нужный момент.
Пример:
